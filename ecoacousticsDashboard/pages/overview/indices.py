...
def update_graph(dates, locations, recorders, feature, normalised, diel_plots, separate_plots):#, time_agg, outliers, colour_locations, ):
    data = filter_dataset(df, dates, feature, locations, recorders)
    data = data.sort_values(by='recorder')

    fig = px.histogram(data, x='value', color='habitat code', marginal='rug', category_orders={'habitat code': ['EC1','EC2','EC3','UK1','UK2','UK3']},
                       facet_col='dddn' if diel_plots else None,
                       facet_row='habitat code' if separate_plots else None,
